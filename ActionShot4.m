function filteredImage = ActionShot4(imagesCells)
...
% takes all the red value of all pixels of all images, find the difference
% between the red value and the median pixel in the corresponding row.
% repeat for blue and green, and sum the squares to calculate squared
% distance (calculating squared distance is faster than non-squared
% distance.
% ultimately, this returns an array which contains the distance between
% every pixel and its corresponding median pixel
distancesFromMedians = (images(:,:,1,:) - medians(:,:,1)).^2 + ...
   (images(:,:,2,:) - medians(:,:,2)).^2 +...
   (images(:,:,3,:) - medians(:,:,3)).^2;
